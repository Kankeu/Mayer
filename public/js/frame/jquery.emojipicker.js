;(function($) {
  ...
  $.extend(Plugin.prototype, {
    ...
    addPickerIcon: function() {
      ...
      this.$wrapper = this.$el
        .wrap("<div class='emojiPickerIconWrap' style='height: 50px'></div>")
        .parent()
      if (this.settings.button) {
        this.$icon = $('<div class="emojiPickerIcon glyphicon glyphicon-apple" style="color: red"></div>')
          .height(iconHeight)
          .width(iconHeight)
    ...
    updatePosition: function() {
      ...
      if (this.settings.container === 'body') {
          top = this.$el.offset().top + this.$el.height() - 330;
          left = this.$el.offset().left + 20;
      }
      else {
    ...
    emojiCategoryClicked: function(e) {
      ...
      this.$picker.find('section.' + section).show().removeClass('hidden');
    },
  ...
  function getPickerHTML() {
    ...
    nodes.push('<div class="emojiPicker" style="overflow: hidden;">');
    nodes.push('<nav>');
    ...
    nodes.push('</nav>');
    for (var i in categories) {
      nodes.push('<section style="overflow-x: hidden" class="' +
        categories[i].name +
        ( i == 0 ? '' : ' hidden' ) +
        '">');
