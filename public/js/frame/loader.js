(function($){
	...
	$('input.round').wrap('<div class="round" style="z-index: 1;position: absolute"/>').each(function(){
		var $input = $(this);
		...
		ctx.beginPath();
		ctx.arc(100,100,60,-1/2 * Math.PI, ratio*2*Math.PI - 1/2 * Math.PI );
		ctx.lineWidth = 10;
		ctx.strokeStyle = color;
		ctx.stroke();

		// Lorsqu'on appuie sur la souris
		$('#loaderjs').keyup(function(event){
			...
			// On écoute le déplacement du curseur
				...
				ctx.beginPath();
				ctx.arc(100,100,60,-1/2 * Math.PI, a*2*Math.PI - 1/2 * Math.PI );
				ctx.lineWidth = 10;
				ctx.strokeStyle = color;
				ctx.stroke();
